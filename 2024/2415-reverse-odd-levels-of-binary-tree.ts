...
function buildTree(arr: number[]): TreeNode | null {
  ...
  const nodes = arr.map(val => new TreeNode(val));
...
function reverseOddLevels(root: TreeNode | null): TreeNode | null {
  ...
  for (let i = 0; queue.length > 0; ++i) {
    if (i % 2) {
      for (let l = 0, r = queue.length - 1; l < r; ++l, --r) {
    ...
    for (const { left, right } of queue) {
      if (left) {
        currentLevel.push(left);
        currentLevel.push(right);
      }
    }
    queue.splice(0, queue.length, ...currentLevel);
  }
  return root;
...
// const root = buildTree([2, 3, 5, 8, 13, 21, 34]);
