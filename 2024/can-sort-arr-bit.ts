...
function canSortArray(nums: number[]): boolean {
  ...
  for (let i = 0; i < n; ) {
    let j = i + 1;
    const currBits = countSetBits(nums[i]);
    let currMin=  nums[i];
    let currMax=  nums[i];
    while (j < n && countSetBits(nums[j]) === currBits) {
...
const unsortedArray = [35,143,127,254];
const sortedArray = canSortArray(unsortedArray);
